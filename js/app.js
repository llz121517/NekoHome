...
/* ---------- 首屏加载遮罩（纯 JS 延迟渲染，开关与文案均来自 config.json） ----------
   config 确认 loading=true 后才创建并显示，文案取 studioName/slogan；
   window load（3s 兜底）后淡出，且自创建起保底显示 300ms 避免一闪而过。 */
function initLoader(cfgPromise) {
  ...
  cfgPromise.then(cfg => {
    ...
    const title = (typeof cfg.studioName === "string" && cfg.studioName) || "NekoHome";
    const subtitle = (typeof cfg.slogan === "string" && cfg.slogan) || "把灵感，做成作品";
    ...
    overlay.innerHTML = `
      <div class="page-loading-content">
        ...
        <div class="page-loading-brand">
          <span class="page-loading-title"></span>
          <span class="page-loading-subtitle"></span>
        </div>
      </div>`;
    overlay.querySelector(".page-loading-title").textContent = title;
    overlay.querySelector(".page-loading-subtitle").textContent = subtitle;
    ...
    Promise.race([loaded, timeout]).then(() => {
      const remain = 300 - (performance.now() - shownAt); // 保底显示 300ms
      setTimeout(() => {
